...
async def screen_custom_resume(file: UploadFile = File(...)):
    """
    Screens an uploaded resume against the active Job Description.
    Accepts PDF, DOCX, PNG, JPG, TXT, or MD. PDFs/images are converted via Mistral's
    OCR API; DOCX is parsed locally with python-docx.
    """
    ...
    try:
        ...
        result = resume_service.screen_custom_resume(filename, file_bytes)
        return result
...
@app.post("/api/feedback/{candidate_id}")
def get_resume_feedback(candidate_id: str):
    """Generates resume-improvement suggestions (strengths, gaps, rewrite tips) for a candidate."""
    try:
        return resume_service.get_resume_feedback(candidate_id)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="Candidate resume not found.")
    except Exception as e:
        logger.error(f"Error generating feedback for {candidate_id}: {e}")
